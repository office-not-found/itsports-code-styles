...
После ввода всех команд выше в корне проекта появится директория `.husky` с git-хуками (по умолчанию будет создан `pre-commit`).
...
### Пример pre-push хука

Помимо `pre-commit`, с помощью Husky можно настроить и `pre-push` хук, который будет выполняться перед `git push`. Например, чтобы не пушить не собирающийся проект, можно добавить хук `pre-push` со следующим содержимым:
